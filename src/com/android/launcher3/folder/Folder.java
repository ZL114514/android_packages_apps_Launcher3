...
import org.avium.launcher.folder.AviumFolderDepthAnimator;
import org.avium.launcher.folder.AviumFolderLayoutSpec;
...
public class Folder extends AbstractFloatingView implements ClipPathView, DragSource,
    ...
    protected void onFinishInflate() {
        ...
        mFolderName.setTypeface(Flags.enableLauncherVisualRefresh()
                ? Typeface.create("google-sans-flex", Typeface.BOLD)
                : Typeface.DEFAULT_BOLD);
        mFolderName.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                dp.getFolderProfile().getLabelTextSizePx()
                        * AviumFolderLayoutSpec.getTitleTextScale());
    ...
    private void animateOpen(List<ItemInfo> items, int pageNo) {
        ...
        boolean isAviumLargeFolder = AviumLargeFolderManager.isLargeFolder(mInfo);
        AnimatorSet animatorSet = getFolderAnimationManager()
                .createAnimatorSet(/* isOpening */ true);
        if (isAviumLargeFolder) {
            AviumLargeFolderManager.hideIconForOpenAnimation(mFolderIcon);
        }
        AviumFolderDepthAnimator.addAnimator(
                animatorSet, mActivityContext, getResources(), /* isOpening */ true);
    ...
    private void animateClosed() {
        ...
        AviumFolderDepthAnimator.addAnimator(
                animatorSet, mActivityContext, getResources(), /* isOpening */ false);
        ...
        if (mFolderIcon != null) {
            ...
            if (wasAnimated) {
                if (AviumLargeFolderManager.isLargeFolder(mInfo)) {
                    AviumLargeFolderManager.prepareIconForTransition(mFolderIcon);
                } else {
    ...
    private void centerAboutIcon() {
        ...
        if (parentWidth <= 0 || parentHeight <= 0) {
            ...
            parentWidth = sTempRect.width();
            parentHeight = sTempRect.height();
        } else {
            sTempRect.set(0, 0, parentWidth, parentHeight);
        }
        int centeredLeft = sTempRect.left + (parentWidth - width) / 2;
        int centeredTop = sTempRect.top + (parentHeight - height) / 2
                - AviumFolderLayoutSpec.getOpenVerticalOffsetPx();
        ...
        mBackground.setBounds(0, getContentAreaTop(), width, height);
    }
    ...
    int getContentAreaTop() {
        return AviumFolderLayoutSpec.getContentAreaTop(getPaddingTop(), mFooterHeight);
    }
    ...
    int getMaxContentAreaHeight() {
        ...
        return grid.getDeviceProperties().getAvailableHeightPx()
                - grid.getWorkspaceIconProfile().getTotalWorkspacePadding().y
                - getFolderNameHeight() - getFooterHeight();
    }
    ...
    int getFolderHeight(int contentAreaHeight) {
        return getPaddingTop() + getPaddingBottom() + getFolderNameHeight()
                + contentAreaHeight + getFooterHeight();
    }
    ...
    private int getFolderNameHeight() {
        return AviumFolderLayoutSpec.getTitleAreaHeight(mFooterHeight);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mFolderName.measure(contentAreaWidthSpec,
                MeasureSpec.makeMeasureSpec(getFolderNameHeight(), MeasureSpec.EXACTLY));
